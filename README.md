...
Определите переменные окружения `SECRET_KEY` и `YANDEX_API_KEY`. API яндекса используется для вычисления расстояния от ресторана до пользователя, создавшего заказ. Создать файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
